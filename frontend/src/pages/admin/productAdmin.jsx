...
export default function ProductAdminPage() {
    ...
    const handleApprovalChange = async (productId, newApprovalStatus, note = undefined) => {
        try {
            ...
            toast.success(`Product ${newApprovalStatus ? 'approved' : 'rejected'} successfully`);
        } catch (error) {
    ...
    const handleNoteUpdate = async (productId) => {
        await handleApprovalChange(productId, product.find(p => p._id === productId)?.isApproved, tempNote);
        setEditingNote(null);
        setTempNote('');
    };
          ...
          <div className="overflow-x-auto">
            ...
            <thead className="bg-gray-100 ">
                <tr>
                    ...
                    <th className="py-3 px-4 text-left font-semibold text-gray-700 border border-gray-300">Note</th>
            ...
            <tbody>
                {filteredProducts && filteredProducts.length > 0 ? filteredProducts.map((products, index) => (
                    <tr key={products._id || index} className="hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 border border-gray-300 whitespace-nowrap">
                            <select
                                ...
                                onChange={(e) => handleApprovalChange(products._id, e.target.value === 'approved')}
                        ...
                        <td className="py-3 px-4 border border-gray-300 min-w-[150px]">
                            {editingNote === products._id ? (
                                ...
                                >
                                    {products.note && products.note !== 'none' ? products.note : 'Click to add note...'}
                                </div>
                            )}
                        </td>
                    ...
                    <tr>
                        <td colSpan="10" className="py-8 px-4 text-center text-gray-500 border border-gray-300">
