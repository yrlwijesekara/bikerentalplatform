...
export default function UpdatebikePage() {
  ...
  const [bikename, setBikename] = useState(location.state.bike.bikeName );
  ...
  const [fuelType, setFuelType] = useState(location.state.bike.fuelType);
  const [suitableTerrain, setSuitableTerrain] = useState(location.state.bike.suitableTerrain);
  ...
  async function handleSubmit(e) {
    ...
    if(imageUrls.length === 0){
      bikeData.images = location.state.bike.images;
    }
    ...
    axios
      .put(import.meta.env.VITE_BACKEND_URL + `/products/${location.state.bike._id}`, bikeData, {
        headers: {
      ...
      <div className="min-h-full flex flex-col items-center justify-center p-4 py-6">
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 flex gap-3 mt-6 pt-4 border-t border-(--brand-primary)">
            ...
            >
              {isSubmitting ? "Updating Bike..." : "Update Bike"}
            </button>
          </div>
        </form>
